...
@Component
public class LexicalSchemaChunkScorer {

    private final TextNormalizer textNormalizer;

    public LexicalSchemaChunkScorer(TextNormalizer textNormalizer) {
        this.textNormalizer = textNormalizer;
    }

    public List<RetrievedChunk> score(ProcessedQuery query, RetrievalMode mode, List<RetrievedChunk> chunks) {
        ...
        for (RetrievedChunk chunk : chunks) {
            scored.add(chunk.withScore(scoreChunk(queryTokens, inferredSchemaRefs, chunk, mode) * correctionPenalty));
        }
        return scored;
    }
